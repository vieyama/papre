...
function nodeTypeLabel(type: NodeType) {
  return type === NodeType.FOLDER ? "Folder" : "Halaman";
}
...
function ListRow({ item, href }: { item: NodeCollectionItem; href: string }) {
  const coverUrl = getCoverUrl(item);
      ...
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {items.length > 0 && (
            ...
            >
              <ToggleGroupItem value="gallery" aria-label="Tampilan galeri">
      ...
      {items.length > 0 ? (
        view === "gallery" ? (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {items.map((item) => (
              <GalleryCard key={item.id} item={item} href={getHref(item)} />
            ))}
          </div>
        ) : (
          <div className="divide-y rounded-xl border">
            {items.map((item) => (
              <ListRow key={item.id} item={item} href={getHref(item)} />
            ))}
          </div>
        )
      ) : (
